...
and **a build is released only on a green qualification.**
...
newest AMD vLLM + ROCm aren't yet usable together on the target hardware. No
release is cut until it goes green on its own; the red result is still recorded
on the qualification feed.
...
2. **Do not attempt to fix a broken upstream release.** If AMD publishes a
   ...
   broken** and no release is cut (the failure is recorded on the qualification
   feed). We never carry a workaround. A red qualification is a correct, useful
   outcome — it tells AMD (and us) the release is not usable, with evidence.
...
5. **Qualification gates release — both channels, green-only.** Every build
   (stable and nightly) is released only when its target's qualification tiers
   pass. There is **no prerelease step and no `publish-prerelease` job**: a
   failed qualification cuts no release at all — the result (pass or fail) is
   recorded on the `qualification-data` feed, and the gfx1151 report is attached
   to the release when one is cut. Lemonade only auto-discovers full releases.
...
- **nightly** repackages AMD's official **universal-RDNA** nightly vLLM
  ...
  no release is cut. We never patch or swap a component to dodge that.
